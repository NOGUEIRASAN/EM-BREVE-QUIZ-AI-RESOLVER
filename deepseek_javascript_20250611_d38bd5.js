(async () => {
    ...
    try {
      ...
      if (toastInstance.toastElement) {
         ...
         toastInstance.toastElement.style.setProperty('--toast-duration', `${duration}ms`);
          ...
          // console.warn("[PREGUISSOS] Não foi possível obter toastElement para aplicar estilos de progresso."); // Log Removido
      }
  ...
  function transformJson(jsonOriginal) {
    if (!jsonOriginal || !jsonOriginal.task || !jsonOriginal.task.questions) {
      console.error("[PREGUISSOS] Estrutura do JSON original inválida para transformação:", jsonOriginal);
    ...
    for (let questionId in jsonOriginal.answers) {
      ...
      if (!taskQuestion) {
        //console.warn(`[PREGUISSOS] Questão com ID ${questionId} não encontrada nos dados da tarefa. Pulando.`); // Log Removido
        continue;
      }
        ...
        switch (taskQuestion.type) {
          case "order-sentences":
            if (taskQuestion.options && taskQuestion.options.sentences && Array.isArray(taskQuestion.options.sentences)) {
              ...
              //console.warn(`[PREGUISSOS] Estrutura inesperada para order-sentences ID ${questionId}`); // Log Removido
            }
            break;
          case "fill-words":
            if (taskQuestion.options && taskQuestion.options.phrase && Array.isArray(taskQuestion.options.phrase)) {
              ...
              //console.warn(`[PREGUISSOS] Estrutura inesperada para fill-words ID ${questionId}`); // Log Removido
            }
            break;
          case "text_ai":
          ...
          case "fill-letters":
            if (taskQuestion.options && taskQuestion.options.answer !== undefined) {
              ...
              //console.warn(`[PREGUISSOS] Estrutura inesperada para fill-letters ID ${questionId}`); // Log Removido
            }
            break;
          case "cloud":
            if (taskQuestion.options && taskQuestion.options.ids && Array.isArray(taskQuestion.options.ids)) {
              ...
              //console.warn(`[PREGUISSOS] Estrutura inesperada para cloud ID ${questionId}`); // Log Removido
            }
            break;
          default:
            if (taskQuestion.options && typeof taskQuestion.options === 'object') {
              ...
              //console.warn(`[PREGUISSOS] Tipo de questão "${taskQuestion.type}" (ID ${questionId}) não possui opções ou estrutura desconhecida.`); // Log Removido
            }
            break;
        }
        novoJson.answers[questionId] = answerPayload;
      } catch (err) {
        console.error(`[PREGUISSOS] Erro ao processar questão ID ${questionId}, tipo ${taskQuestion.type}:`, err);
  ...
  async function pegarRespostasCorretas(taskId, answerId, headers) {
    ...
    //console.log("[PREGUISSOS] Buscando respostas. Headers:", headers); // Log Removido
    sendToast("Buscando respostas corretas...", 2000);
    try {
      const response = await fetch(url, { method: "GET", headers: headers });
      if (!response.ok) {
        console.error(`[PREGUISSOS] Erro ${response.status} ao buscar respostas. URL: ${url}`);
        throw new Error(`Erro ${response.status} ao buscar respostas.`);
      }
      const data = await response.json();
      //console.log("[PREGUISSOS] Respostas corretas recebidas:", data); // Log Removido
      return data;
    } catch (error) {
      console.error("[PREGUISSOS] Falha detalhada ao buscar respostas corretas:", error);
  ...
  async function enviarRespostasCorrigidas(respostasAnteriores, taskId, answerId, headers) {
    ...
    try {
      ...
      //console.log("[PREGUISSOS] Enviando respostas. Payload:", JSON.stringify(novasRespostasPayload, null, 2)); // Log Removido
      //console.log("[PREGUISSOS] Enviando respostas. Headers:", headers); // Log Removido
      ...
      if (!response.ok) {
        ...
        console.error(`[PREGUISSOS] Erro ${response.status} no PUT. URL: ${url}. Response Body:`, errorBody);
        try { errorBody = JSON.parse(errorBody); } catch (e) {}
        throw new Error(`Erro ${response.status} ao enviar respostas.`);
      }

      //console.log("[PREGUISSOS] Respostas corrigidas enviadas com sucesso."); // Log Removido
      ...
      console.error("[PREGUISSOS] Falha detalhada ao transformar ou enviar respostas corrigidas:", error);
  ...
  try {
    ...
    sendToast("Créditos: IGUINHO PARAGUAI", 5000);

  } catch (error) {
    console.error("[PREGUISSOS] Falha ao carregar dependências (Toastify ou Fonte):", error);
  ...
  window.fetch = async function(input, init) {
    ...
    //console.log(`[PREGUISSOS] Fetching: ${method} ${url}`); // Log Removido

    if (url === 'https://edusp-api.ip.tv/registration/edusp/token' && !capturedLoginData) {
       //console.log('[PREGUISSOS] Interceptando requisição de token...'); // Log Removido
      try {
        const response = await originalFetch.apply(this, arguments);
        const clonedResponse = response.clone();
        const data = await clonedResponse.json();
        //console.log("[PREGUISSOS] Resposta da API de Token Recebida:", data); // Log Removido

        if (data && data.auth_token) {
          capturedLoginData = data;
          //console.log("[PREGUISSOS] >>> Token Capturado com Sucesso:", capturedLoginData.auth_token); // Log Removido
          //console.log("[PREGUISSOS] Dados completos de login capturados:", capturedLoginData); // Log Removido
          ...
          if (isToastifyLoaded) {
            ...
            if (firstName) {
              ...
              //console.warn("[PREGUISSOS] Não foi possível encontrar ou processar 'name' na resposta do token."); // Log Removido
            }
           ...
           console.warn("[PREGUISSOS] Resposta do token recebida, mas 'auth_token' não encontrado na estrutura esperada:", data); // Mantido Warn importante
           if (isToastifyLoaded) {
        ...
        console.error('[PREGUISSOS] Erro CRÍTICO ao processar resposta do token:', error); // Mantido Erro Crítico
        if (isToastifyLoaded) {
    ...
    if (answerSubmitRegex.test(url) && init && init.method === 'POST') {
      //console.log("[PREGUISSOS] Interceptando envio de resposta POST..."); // Log Removido
      //console.log("[PREGUISSOS] Verificando token antes de corrigir. capturedLoginData existe?", !!capturedLoginData); // Log Removido

      if (!capturedLoginData || !capturedLoginData.auth_token) {
        //console.warn("[PREGUISSOS] Tentativa de correção, mas o token ainda não foi capturado ou é inválido."); // Log Removido
        if (isToastifyLoaded) {
      ...
      try {
        ...
        //console.log("[PREGUISSOS] Resposta do POST original enviada detectada:", submittedData); // Log Removido

        if (submittedData && submittedData.status !== "draft" && submittedData.id && submittedData.task_id) {
           //console.log("[PREGUISSOS] Status OK para correção automática. Iniciando processo..."); // Log Removido
          ...
          //console.log("[PREGUISSOS] Headers para correção:", headers_template); // Log Removido

          setTimeout(async () => {
             //console.log("[PREGUISSOS] Executando correção após delay..."); // Log Removido
            try {
              ...
              //console.error("[PREGUISSOS] Erro final capturado durante o processo de correção automática:", correctionError); // Log Removido
            }
          }, 500);

        } else {
          //console.log("[PREGUISSOS] Envio detectado, mas não requer correção (status draft ou dados faltando). Status:", submittedData?.status, "ID:", submittedData?.id, "Task ID:", submittedData?.task_id); // Log Removido
        }
      } catch (err) {
        console.error('[PREGUISSOS] Erro ao processar a resposta JSON do envio de tarefa POST:', err); // Mantido Erro importante
        if (isToastifyLoaded) {
  ...
  console.log(`[PREGUISSOS] ${SCRIPT_NAME}: Interceptador de fetch ativo.`);
